...
(function () {
    ...
    // Gemini API Configuration (Direct IP for troubleshooting DNS issues)
    const GEMINI_API_URL = 'https://164.90.186.94/api/chat';
    ...
    async function getAIResponse(userMessage) {
        try {
            ...
            if (!response.ok) {
                ...
                const msg = errorData.error?.message || errorData.details || `Error ${response.status}`;
                throw new Error(msg);
            }
            ...
            if (!data.candidates || data.candidates.length === 0) {
                if (data.promptFeedback && data.promptFeedback.blockReason) {
                    throw new Error(`Безопасность: Ответ заблокирован (${data.promptFeedback.blockReason})`);
                }
                throw new Error('ИИ не смог сгенерировать ответ. Попробуйте другой вопрос.');
            }
